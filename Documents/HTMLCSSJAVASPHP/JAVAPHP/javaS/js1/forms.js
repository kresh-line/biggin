var f = document.getElementById("formtocheck1");
...
function checkform1(e) {
    let msgstr = "";
    if (f1.elements['fruit'].checkValidity() === false) {
        if (f1.elements['fruit'].validity.patternMismatch === true) {
            msgstr = msgstr + "Παρακαλώ επιλέξτε μπανάνα ή κεράσι.<br>";
        }
        if (f1.elements['fruit'].validity.valueMissing === true) {
            msgstr = msgstr + "Παρακαλώ επιλέξτε ένα φρούτο.<br>";
        }
    }
    if (f1.elements['email'].checkValidity() === false) {
        msgstr = msgstr + "bale to email.<br>";
    } else if (f1.elements['email'].value.endsWith("@gmail.com") === false) {
        msgstr = msgstr + "be email mono sto @gmail.com.<br>";
    }
    let num = 0;
    if (f1.elements['choice1'].checked === true) num++;
    if (f1.elements['choice2'].checked === true) num++;
    if (f1.elements['choice3'].checked === true) num++;
    if (num < 2) {
        msgstr = msgstr + "Παρακαλώ επιλέξτε τουλάχιστον 2 επιλογές.<br>";
    }
    //e.preventDefault();
    ...
    //else {
    //    errorform1.innerHTML = "U dërgua me sukses!";
    //}
}